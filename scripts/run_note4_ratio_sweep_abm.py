"""
Note04（04_Sensitivity_Chi_Landscape）补强：媒体生态比值 (n_w/n_m) 的 ABM 对照扫参

目标：
- 对 Part 3 的理论曲线 rc(n_w/n_m) 给出 ABM 对照：在对称验证口径（symmetric_mode=True, beta=0）下，
  用“最大斜率法（含局部二次插值）”估计有效临界点 rc，并 bootstrap 给出 95% 区间。

设计原则：
- 只改变 ratio=n_w/n_m；全局信号里同时缩放 n_m,n_w 不影响比例，因此默认用 n_m=1.0，n_w=ratio。
- k_avg 固定，用于计算 chi_ref；ABM 中 sample_n 也固定为同一个 k_avg，以保持“理论↔模拟”的映射一致。

用法示例（工作站）：
python3 scripts/run_note4_ratio_sweep_abm.py \\
  --jobs 48 \\
  --ratio-list 0.1,0.2,0.3,0.5,0.8,1.0,1.2,1.5,2.0 \\
  --k-avg 50 \\
...
def _simulate_one_seed(task: SeedTask) -> Tuple[float, int, np.ndarray]:
    """返回：(ratio, seed, q_abs_mean_over_r)"""
...
def main() -> None:
    p = argparse.ArgumentParser(description="Run Note04 Part3 ABM ratio sweep and save .npz")
    p.add_argument("--phi", type=float, default=0.54)
    p.add_argument("--theta", type=float, default=0.46)
    p.add_argument("--k-avg", type=int, default=50)
    p.add_argument("--ratio-list", type=str, default="0.1,0.2,0.3,0.5,0.8,1.0,1.2,1.5,2.0")
    ...
    ratio_list = _parse_float_list(args.ratio_list)
    if not ratio_list:
        raise SystemExit("ratio-list 不能为空")
    seeds = _parse_int_list(args.seeds)
    if not seeds:
        raise SystemExit("seeds 不能为空")
    r_vals = np.linspace(float(args.r_min), float(args.r_max), int(args.r_num), dtype=float)

    chi_ref = float(calculate_chi(phi=args.phi, theta=args.theta, k_avg=int(args.k_avg)))
    rc_theory = np.array([float(calculate_rc(n_m=1.0, n_w=float(ratio), chi=chi_ref)) for ratio in ratio_list], dtype=float)
    ...
    print(
        f"scan ratio={ratio_list}, r=[{args.r_min},{args.r_max}] x{r_vals.size}, "
        f"k_avg={args.k_avg}, n={args.n}, deg={args.avg_degree}, model={args.model}, "
        f"steps={args.steps}, ri={args.record_interval}, burn={args.burn_in_frac}, win={args.metric_window}, "
        f"seeds={len(seeds)}, jobs={args.jobs}"
    )
    print(f"theory chi_ref={chi_ref:.6f}")

    nP = len(ratio_list)
    nR = int(r_vals.size)
    nS = len(seeds)
    q_abs = np.full((nP, nR, nS), np.nan, dtype=float)

    idx_ratio = {float(r): i for i, r in enumerate(ratio_list)}
    ...
    def run_sequential() -> None:
        ...
        for t in tasks:
            ratio, seed, q_abs_s = _simulate_one_seed(t)
            ri = idx_ratio[float(ratio)]
    ...
    if int(args.jobs) <= 1:
        ...
        try:
            with ProcessPoolExecutor(max_workers=int(args.jobs)) as ex:
                futs = {ex.submit(_simulate_one_seed, t): (t.ratio, t.seed) for t in tasks}
                done = 0
                for fut in as_completed(futs):
                    ratio, seed, q_abs_s = fut.result()
                    ri = idx_ratio[float(ratio)]
                    si = idx_seed[int(seed)]
                    q_abs[ri, :, si] = q_abs_s
                    done += 1
    ...
    rc_est = np.full(nP, np.nan, dtype=float)
    rc_ci_low = np.full(nP, np.nan, dtype=float)
    rc_ci_high = np.full(nP, np.nan, dtype=float)
    rc_valid_boot = np.zeros(nP, dtype=int)

    for ri, ratio in enumerate(ratio_list):
        med, lo, hi, valid = _bootstrap_rc(
            r_vals,
            q_abs[ri, :, :],
            n_boot=int(args.bootstrap),
            seed=20251215 + int(round(float(ratio) * 10_000)),
            min_delta=float(args.min_delta),
        )
        rc_est[ri] = med
        rc_ci_low[ri] = lo
        rc_ci_high[ri] = hi
        rc_valid_boot[ri] = int(valid)
        print(f"[rc] ratio={float(ratio):.3g}: rc_maxslope={med:.4f} CI95%=[{lo:.4f},{hi:.4f}] (valid_boot={valid}/{int(args.bootstrap)})")
