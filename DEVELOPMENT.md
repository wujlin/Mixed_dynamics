...
> - 结构性效应（H2）目前在 **batch3（按周分段，freq=4H）** 上更清晰：`r_proxy_mean ↑ → volatility ↑`（相关显著），但需要对“段内样本量/密度混杂”做稳健性说明（partial 相关会变弱）。
> - 动力学效应（H1）在混合池 `all` 的周分段上出现正相关信号（Pearson 显著），但在单词条 batch1 上仍偏弱，可能受功效不足与 jump 定义影响。
> - H4（CSD 预警）对连续性与密度敏感：master 过稀疏时严格口径下不可评估；batch3/all 可评估但证据仍不稳健，建议定位为探索性结果或局限性讨论。
