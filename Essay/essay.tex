...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    ...
    \caption{}
  \end{subfigure}
  \caption{Empirical signatures at the segment level. (a) Activity $a$ versus jump intensity $\text{jump}_{q95}$ in the pooled dataset (see Methods for definitions). (b) $r_{\text{proxy}}$ versus volatility in the high-density subset (12-hour aggregation); color indicates segment sampling density (median split). (c) Representative high- and low-activity segments; vertical dashed lines mark the maximal single-step change in $|Q|$ within each segment. The high-activity segment (blue) exhibits larger polarization swings. (d) Representative high- and low-$r_{\text{proxy}}$ segments; $Q$ is shown after subtracting the segment mean to facilitate volatility comparison. The high-$r_{\text{proxy}}$ segment (orange) exhibits greater fluctuation amplitude.}
  \label{fig:empirical}
\end{figure}
