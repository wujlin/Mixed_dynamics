...
% Robust figure paths for either:
% (a) Overleaf project root contains figures/ and figures_supp/, or
% (b) this file lives under Essay/slides/ with figures in ../figures/.
\IfFileExists{figures/fig0_framework.pdf}{%
...
\title[Mixed-feedback transitions]{Phase Transitions in Collective Emotion:\\A Mixed-Feedback Framework Motivated by Media Dynamics}
\author{Author A}
\institute{Internal discussion with partners}
\date{\today}
...
\begin{frame}
  ...
  \begin{block}{One-sentence takeaway}
    Channel balance controls collective emotional resilience: mixed feedback yields a direction--intensity transition, and elevated activity can act as a detectable precursor.
  \end{block}
\end{frame}
...
\begin{frame}{Roadmap: a theory-to-data evidence chain}
  ...
  \vspace{0.4em}
  \small Theory (bifurcation, CSD) $\rightarrow$ Network validation $\rightarrow$ Parameter landscape $\rightarrow$ Empirical signatures
\end{frame}
...
\begin{frame}{Parameter landscape: where is the system fragile?}
  ...
  \end{itemize}
  \centering
  \includegraphics[height=0.62\textheight,keepaspectratio]{fig4a_chi_rc_landscape.pdf}
\end{frame}
...
\begin{frame}{Structural levers: local coupling}
  ...
  \end{itemize}
  \centering
  \includegraphics[height=0.60\textheight,keepaspectratio]{fig4d_beta_effect.pdf}
\end{frame}
...
\begin{frame}{Takeaways \& call-to-action}
  \begin{block}{Core contributions (what is new)}
    \begin{itemize}
      ...
      \item A multi-level evidence chain: theory $\rightarrow$ network validation $\rightarrow$ parameter landscape $\rightarrow$ empirical signatures.
    \end{itemize}
  \end{block}
  \begin{block}{Decisions / help needed}
    \begin{itemize}
      \item Narrative check: is the evidence chain convincing and easy to follow for a non-expert reader/editor?
      \item Empirical robustness: any preferred alternative density controls or subset definitions?
      \item Submission strategy: confirm target journal shortlist (Nature sub-journal track).
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Backup: H2 density diagnostics (4H vs 12H)}
  \small 12H aggregation stabilizes within-segment estimates under missingness; 4H diagnostics are more sensitive to sparse coverage.\par
  \vspace{0.3em}
  \centering
  \includegraphics[height=0.36\textheight,keepaspectratio]{s5_h2_density_4h.pdf}\par
  \vspace{0.2em}
  \includegraphics[height=0.36\textheight,keepaspectratio]{s5_h2_density_12h.pdf}
\end{frame}
...
\begin{frame}{Backup: LLM annotation (summary)}
  \begin{itemize}
    ...
    \item Human validation subset ($n=3{,}000$): 83\% exact-match agreement (overall accuracy).
  \end{itemize}
\end{frame}
