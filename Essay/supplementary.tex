...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.92\linewidth}
    ...
    \caption{}
  \end{subfigure}
  \caption{Density diagnostics for the media-dominance--volatility signature under two aggregation scales (Dataset B / batch3). \textbf{Top:} 4-hour aggregation, showing a positive raw association (Pearson $r=0.265$, $p=0.00434$) that attenuates after controlling for segment sampling density (partial $r=0.078$, $p=0.411$; median split at 28 windows). \textbf{Bottom:} 12-hour aggregation, yielding a stronger positive association that remains significant after density control (Pearson $r=0.429$, $p=6.03\times 10^{-7}$; partial $r=0.386$, $p=8.79\times 10^{-6}$; median split at 14 windows). Solid/dashed lines show groupwise linear fits with 95\% bootstrap bands.}
