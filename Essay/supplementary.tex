...
This note provides robustness checks supporting the theoretical and network-validation results in the main text. The additional analyses below serve three narrative purposes: (i) justify the order-parameter conventions used to visualize the transition, (ii) validate that the activity variable carries independent dynamical information under realistic asymmetry, and (iii) motivate the finite-size estimator used to locate the transition in sparse networks.
...
The main text reports a non-monotonic $k$ effect on the estimated transition point. We separate this into (i) how $k$ changes the psychological sensitivity $\chi$ and (ii) how the induced $\chi(k)$ maps to $r_c(k)$.
...
To connect directly to the theoretical mechanism, we evaluate two segment-level associations: (i) elevated activity predicting larger polarization jumps, and (ii) increased user-generated dominance predicting higher polarization volatility. Density stratification is shown to diagnose coverage-driven confounds.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\linewidth]{s5_scatter_h1_h2.pdf}
  \caption{Scatter diagnostics for empirical signatures across datasets (Dataset A/B/C as defined above). We show the segment-level associations used for the activity--jump and media-dominance--volatility tests, including density-stratified views used to diagnose sampling-coverage confounds.}
...
\subsection{Robustness to temporal aggregation (12H)}
Finally, we repeat the media-dominance--volatility test under 12-hour aggregation, which increases per-window sample size at the cost of temporal granularity. The positive association remains, supporting that the main-text pattern is not an artifact of a single aggregation choice.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\linewidth]{s5_h2_density_12h.pdf}
  \caption{Robustness to temporal aggregation for the media-dominance--volatility signature. Repeating the analysis under 12-hour aggregation increases per-window sample size and yields a similar positive association between $r_{\text{proxy}}$ and volatility (Pearson $r=0.429$, $p=6.03\times 10^{-7}$; partial $r=0.386$, $p=8.79\times 10^{-6}$ controlling for the number of valid windows per week; median split at 14 windows). Solid/dashed lines show groupwise linear fits with 95\% bootstrap bands.}
