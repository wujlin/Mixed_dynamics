...
\begin{abstract}
\noindent
Online collective emotion can shift abruptly from calm to polarized, yet the structural conditions governing these transitions remain unclear. Here we show that competition between stabilizing mainstream and amplifying user-generated media defines a critical stability boundary parameterized by a single channel-balance ratio. We derive this boundary analytically and show that near criticality, collective engagement activity rises before polarization direction is selected---a model-derived early-warning indicator. Agent-based simulations confirm that the transition structure persists on heterogeneous networks under finite-size fluctuations. In social-media data from the COVID-19 period, we find both predicted signatures: elevated activity precedes larger polarization jumps, and greater user-generated dominance associates with higher volatility. These results suggest that collective fragility is governed not only by content extremity but by the structural balance between feedback channels---a balance that is measurable and, in principle, governable.
\end{abstract}
...
\label{sec:results}
% ════════════════════════════════════════════════════════════════════
\noindent The results follow a theory-to-data chain. We show that mixed feedback yields a direction--intensity transition with bistability; the parameter landscape identifies which structural configurations are most vulnerable; and both implied signatures---elevated activity predicting larger polarization jumps, and user-generated dominance covarying with higher volatility---appear in social-media data.
...
The analytical framework connects directly to measurable quantities: the channel-balance ratio $r$ maps to observable media-composition metrics, and critical slowing down translates to rising autocorrelation in time-series data---computable without platform access. This tractability complements the data-driven approaches that dominate computational studies of online polarization, offering a mechanistic link between platform architecture and collective dynamics.
