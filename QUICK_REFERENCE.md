...
# Windows
powershell -ExecutionPolicy Bypass -File verify_gitignore.ps1
```
...
```bash
# 添加修改
git add .gitignore README.md CODE_STRUCTURE.md
...
| **开发** | `AGENTS.md`, `DEVELOPMENT.md` | 内部文档 |
...
| **测试** | `tests/` | 单元测试 |
| **文档** | `README.md`, `CODE_STRUCTURE.md` | 项目说明 |
...
- [ ] 运行 `verify_gitignore.sh` 或 `verify_gitignore.ps1`
- [ ] 所有检查项通过（0 个失败）
- [ ] `git status --ignored` 显示敏感文件被忽略
- [ ] `git status` 只显示应该提交的文件
- [ ] `data/derived/` 中的数据文件无用户信息
- [ ] `README.md` 和 `CODE_STRUCTURE.md` 已更新
...
- **验证脚本**: `verify_gitignore.sh` / `verify_gitignore.ps1`
