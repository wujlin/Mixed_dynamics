...
### 6.6 参数重要性与交互：Δ$r_c$ 口径的杠杆对比 + $k\times(n_w/n_m)$（理论）

我们进一步把 Note04 的多参数扫描结果压缩成两个可直接服务写作的结论：
1) **参数“杠杆大小”**：用全扫描范围的 $\Delta r_c=\max(r_c)-\min(r_c)$（并对 $r_{c,\mathrm{base}}$ 归一化）衡量“改变结构条件能把系统推离/推近临界边界的幅度”。  
2) **交互效应（理论层）**：媒体生态比值 $n_w/n_m$ 的作用强弱会被心理敏感度 $\chi$ 调制；而 $\chi$ 又随信息密度 $k$ 改变，因此存在一个可用于 policy framing 的 $k\times(n_w/n_m)$ 交互。

**补充图 18（Note04）：参数重要性（哑铃图）与 $k\times(n_w/n_m)$ 理论交互**  
<img src="../outputs/figs/supp/fig_supp_param_importance_preview.png" width="980">

**补充图 18 的含义**
- （a）在本扫描口径下，**媒体生态比值 $n_w/n_m$ 与局部耦合 $\beta$ 的杠杆最大**：二者都能带来约 $\Delta r_c\approx 0.50$（约为基准 $r_c$ 的 2/3）的边界平移；阈值 $(\phi,\theta)$ 次之；信息密度 $k$ 的直接平移幅度较小。  
- （b）理论交互显示：当 $\chi(k)$ 更高（更“敏感”的系统）时，$r_c(n_w/n_m)$ 曲线更陡，意味着**相同幅度的媒体结构变化会造成更大的脆弱性迁移**。这提供了一个直观的“何时干预更有效”的 framing：敏感度更高的语境里（例如更强烈的集体风险感/更高的信息密度），结构性调参对韧性的影响更显著。

**关键数值表（Δ$r_c$ / $r_{c,\mathrm{base}}$ 口径）**

| Factor | Scan range | $r_{c,\min}$ | $r_{c,\max}$ | $r_{c,\mathrm{base}}$ | $\Delta r_c$ | $\Delta r_c/r_{c,\mathrm{base}}$ |
|---|---:|---:|---:|---:|---:|---:|
| Media ratio $n_w/n_m$ | 0.1–2.0 | 0.433 | 0.939 | 0.753 | 0.506 | 67.1% |
| Local coupling $\beta$ (ABM) | 0.00–0.20 | 0.250 | 0.752 | 0.752 | 0.502 | 66.7% |
| Thresholds $(\phi,\theta)$ | grid (81×81) | 0.743 | 0.989 | 0.753 | 0.246 | 32.7% |
| Information density $k$ | 10–500 | 0.737 | 0.826 | 0.753 | 0.088 | 11.7% |

**Policy mapping（写作素材，主文可择要引用）**
- $n_w/n_m$（媒体生态）对应“通道权重/曝光结构”：更偏向自媒体放大（更大 $n_w/n_m$）会系统性降低 $r_c$，意味着更易失稳；在平台层面可通过曝光混合、算法增益调节等改变“有效通道权重”。  
- $\beta$（局部耦合）对应“邻域强化/同伴可见性”：它作用层级不同于 $n_w/n_m$，主要通过网络结构与局部互动放大来移动“有效转变点”，提示邻居状态可见性与局部推荐机制也是独立杠杆。  
- $(\phi,\theta)$ 与 $k$ 更像系统固有属性/语境因素：它们决定“敏感度与噪声结构”，从而调制上述结构杠杆的有效性（见 panel b 的交互）。
