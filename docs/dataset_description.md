...
数据集时间跨度较长（**master** 覆盖约 2015-2024；**Batch3 扩展集** 覆盖约 2020-2025），包含微博内容、发布时间、用户信息及情绪/风险标注。实际用于窗口化分析时，数据有效密度主要集中在 2022-2024（尤其 Batch3 的 2023-2024 更密集）。为降低“时间窗缺口/稀疏”带来的统计伪影，经验验证（Note07）默认先聚焦 2023+（可在 notebook 里调 `TIME_START/TIME_END` 做稳健性对照）。
