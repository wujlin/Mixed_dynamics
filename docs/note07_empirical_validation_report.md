...
> **PI 收敛决策（2025-12-20）**：经验部分采用 **Two‑Tier Validation** 叙事并停止新增迭代。  
...
## 4. 核心发现（给写作的一句话版本）
1) **H1（Activity → Jump）在 `all` 上得到经验支持**：在 `all`（master+batch3 合并）按周分段的主口径下，H1 为正且显著（Pearson r=0.241, p=0.00798）。这支持“更高活跃度对应更强的变化幅度/更大 jump”的方向性结论（作为内生脆弱性的经验信号）。  
2) **H2（r_proxy → Volatility）在 `batch3` 上显著，但密度是重要混杂因素**：`batch3` 上 H2 显著为正（Pearson r=0.265, p=0.00434；Spearman r=0.244, p=0.00902），但控制段内有效窗口数 `n_windows_aq` 后的部分相关不显著（r=0.078, p=0.411）。论文应如实披露“密度/可用窗口结构”对 H2 的影响，并把它作为稳健性与局限性的一部分。对应可视化：`outputs/figs/empirical/fig7b_h2_scatter_batch3_density_4h.png`。  
3) **H4（CSD/early warning）在当前数据分辨率下不定论**：在 4H 主口径下，H4 的 Placebo p 值整体不显著；当尝试提频到 2H/1H 时，连续性约束会导致 eligible=0、events=0（不可评估）。因此更合适的写法是“理论上存在 CSD，但在当前社交媒体数据连续性/功效限制下难以稳定捕捉”，并把 1H/2H 失败作为重要的 invalidation finding 写入附录。
